...
class DEBUGSentContr extends Controller
{
    public function getEmotions($input_tweets){
       ...
       //FOR THE DEBUG VERSION ONLY
		$file = 'DEBUG_SCAN.txt';
		file_put_contents($file, "RUNNING NEW SCAN"); //JUST CLEAR IT OUT EVERY TIME WE RUN A NEW SCAN
		...
		foreach ($SAMPLE_TWEETS as $item) {
			...
			$current .= "DP -- $tweet \n";
			// // Write the contents back to the file  //NOT QUITE YET...
		...
		if ($DEBUG == true){
			...
			*/
			...
			foreach ($all_tops as $key => $category) {
				foreach (array_reverse($category, true) as $toptweet) {  		//CYCLE THROUGH THE LIST OF MOST EMOTIONAL TWEETS, GREATEST TO SMALLEST
					if ((in_array($toptweet, $emo_used_list)) == false) { 		//IF THIS TWEET HASN'T WON A CATEGORY YET -
						...
						$current .= "MOST EMOTIONAL $key -- $toptweet \n";
			...
			//WE SHOULDNT NEED TO RETURN ANYTHING IN THIS MODE
